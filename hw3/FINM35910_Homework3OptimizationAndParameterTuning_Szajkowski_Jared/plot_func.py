...
def plot_cumulative_return(strat_df, fund_list, initial_investment = 10000, export_plot = "Yes"):
    ...
    # Plotting data
    plt.plot(strat_df.index, strat_df['Cumulative_Return'], label = "Trend Following Model Cumulative Return", linestyle = "-", color = "red", linewidth = 1)
...
def plot_values(strat_df, export_plot = "Yes"):  
    # Generate plot   
    plt.figure(figsize=(10, 5), facecolor = "#F5F5F5")
    ...
    plt.plot(strat_df.index, strat_df['Total_Value'], label = "Total Portfolio Value", linestyle = "-", color = "red", linewidth = 1)
...
def plot_buy_hold(strat_df, fund_list, initial_investment = 10000, export_plot = "Yes"): 
    # Create dataframe for prices
    buy_hold = pd.DataFrame(index = strat_df.index)
    ...
    # Plotting data
    plt.plot(strat_df.index, strat_df['Total_Value'], label = "Trend Following Model Total Portfolio Value", linestyle = "-", color = "red", linewidth = 1)
    for idx, fund in enumerate(fund_list):
...
def plot_drawdown(strat_df, fund_list, initial_investment = 10000, export_plot = "Yes"): 
    # Create dataframe for prices
    buy_hold = pd.DataFrame(index = strat_df.index)
    ...
    min_value = drawdown.min() # Set minimum value to start with which will be used later on for the plot Y axis
    ...
    for idx, fund in enumerate(fund_list):
        ...
        if drawdown.min() < min_value:
            min_value = drawdown.min() # Set min value if lower than strategy drawdown above
        else:
            pass
...
def plot_price(strat_df, fund, export_plot = "Yes"):   
    # Generate plot   
    plt.figure(figsize=(10, 5), facecolor = "#F5F5F5")
    ...
    # Save the figure
    if export_plot == "Yes":
        plt.savefig("Crypto_Trend_Following/Portfolio_" + fund + "_Price_History.png", dpi = 300, bbox_inches = "tight")
        # plt.savefig("Portfolio_" + fund + "_Price_History.png, dpi = 300, bbox_inches = "tight")
    else:
        pass
...
def plot_cum_trans_costs(strat_df, export_plot = "Yes"):  
    # Generate plot   
    plt.figure(figsize=(10, 5), facecolor = "#F5F5F5")
    ...
    # Save the figure
    if export_plot == "Yes":
        plt.savefig("Crypto_Trend_Following/Portfolio_Cumulative_Transaction_Costs.png", dpi = 300, bbox_inches = "tight")
        # plt.savefig("Portfolio_Cumulative_Transaction_Costs.png", dpi = 300, bbox_inches = "tight")
    else:
        pass
